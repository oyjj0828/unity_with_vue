    ...
    <div class="boxall" style="height: calc(100% )">
      ...
      <div class=" boxnav " id="echarts4" style="user-select: none; z-index: -10;"
        ...
        <div class="mainbox" style="position: relative; overflow:hidden; width: 100%; height: 100%;
                        cursor: default;">
          <div ref="chart3" style="width: 100%; height: 100%; top:15px"></div>
        </div>
      </div>
    </div>
  </dv-border-box-13>
...
export default {
  ...
  methods: {
    async fetchDataAndUpdate3() {
      await axiosInstance.get('/next-nt-records', {
        params: { batch_size: 5, model: this.$store.state.tempe_model }
      })
        .then(response => {
    ...
    async initChart3() {
      await axiosInstance.get('/next-nt-records', {
        params: { batch_size: this.batchSize ,init: 1, model: this.$store.state.tempe_model}
      })
        .then(response => {
          this.records = response.data;
          this.consumption = this.records.map(record => record.Consumption)
          ...
          axiosInstance.get('/next-ant-records', {
            params: { batch_size: this.batchSize1 , init: 1, model: this.$store.state.tempe_model}
          })
            .then(response => {
              this.records1 = response.data;
              this.consumption1 = this.records1.map(record => record.Consumption)
